...
const children = procs.map((p) => spawn(p.cmd[0], [...p.cmd.slice(1), String(p.port)], { stdio: "ignore" }));
const MB = 1048576;
try {
  for (const p of procs) await waitReady(p.port);
  const idle = {};
  for (const p of procs) idle[p.name] = await (await fetch(`http://127.0.0.1:${p.port}/debug/memory`)).json();
  ...
  for (let round = 0; round < 3; round++) {
    for (const p of procs) {
      ...
      await autocannon({ url: `http://127.0.0.1:${p.port}/user`, connections: 200, duration: 8, pipelining: 1, workers: 4, warmup: { connections: 200, duration: 2 } });
      stop.done = true;
      await sampler;
  ...
  const steady = {};
  for (const p of procs) steady[p.name] = await (await fetch(`http://127.0.0.1:${p.port}/debug/memory`)).json();
  console.log(`\nidle:  keala ${(idle.keala.rss/MB).toFixed(1)}MB  hono ${(idle.hono.rss/MB).toFixed(1)}MB`);
  console.log(`steady:keala ${(steady.keala.rss/MB).toFixed(1)}MB  hono ${(steady.hono.rss/MB).toFixed(1)}MB`);
  console.log(`peak:  keala ${(peaks.keala/MB).toFixed(1)}MB  hono ${(peaks.hono/MB).toFixed(1)}MB`);
} finally { for (const c of children) c.kill("SIGKILL"); }
