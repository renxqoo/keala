...
- Runtimes: bun 1.4.0 (raw / bun-koa / hono) vs node 22 (koa / fastify) vs go 1.27
...
- bun-koa vs raw Bun: **1.00x** (±7% / ±5%)
- bun-koa vs go net/http: **1.27x** (±7% / ±3%)
...
- bun-koa vs raw Bun: **0.97x** (±8% / ±9%)
- bun-koa vs go net/http: **1.25x** (±8% / ±3%)
...
- bun-koa vs raw Bun: **0.98x** (±6% / ±5%)
- bun-koa vs go net/http: **1.24x** (±6% / ±3%)
...
- bun-koa vs raw Bun: **0.86x** (±3% / ±1%)
- bun-koa vs go net/http: **1.11x** (±3% / ±1%)
...
- bun-koa vs raw Bun: **1.38x** (±3% / ±2%)
- bun-koa vs go net/http: **1.31x** (±3% / ±2%)
...
- **8-core Apple Silicon** (client and servers co-resident, headroom to
  spare): the original throughput headline — `bun-koa` ≈ 200–243k req/s.
...
- **10-core Apple M4** (the report above): the post-hardening regression
  check machine — `bun-koa` ≈ 226–247k req/s, with the Go reference
...
- **Idle memory**: bun-koa 24–25MB vs hono 25–26MB on the M4 — the lazy
...
dispatch at 1000 entries; M4 run 2: bun-koa 247k vs raw 179k, ±3%/±2%). The
...
trails Go on the same box, and bun-koa adds nothing to that gap versus hono
