...
console.log(`raw   Response (pooled): content-type=${JSON.stringify(a.headers.get("content-type"))} status=${a.status}`);
console.log(`sugar c.text   (pooled): content-type=${JSON.stringify(b.headers.get("content-type"))} status=${b.status}`);
process.exit(0);
