...
| `cors` / `csrf`                       | credentials require an origin whitelist; reflected origins always carry `Vary: Origin` (a constant `*` answer doesn't — it never varies); `Origin: null` rejected; genuine preflights (with `Access-Control-Request-Method`) only; `origin` also accepts a per-request predicate; preflight ACRH reflection is opt-in (`reflectHeaders`); `csrf({ allow })` exempts authenticated non-browser callers BEFORE the origin check |
| `etag` / `compress`                   | weak tags + 304 (If-None-Match precedence per RFC 9110); async gzip, injected for tests                                                                                                                                                                                                                                                                                                                                       |
| `serveStatic`                         | decode → normalize → containment; null bytes 400; ANY symlink component denied (dirs included); index containment re-checked; `precompressed` serves `.br`/`.gz` siblings (brotli first, `Content-Encoding` + `Vary: Accept-Encoding`); `onFound`/`onNotFound` hooks                                                                                                                                                          |
| `rateLimit`                           | fixed window per key (default `c.ip`); 429 + `Retry-After` + `RateLimit-*` headers; shared stores implement the atomic `{ hit, get }` interface — **0.8 breaking: a bare `Map` is no longer accepted** (get/set read-modify-write races workers past the limit); wrap one with `memoryRateLimitStore(maxKeys)`, Redis adapters map to `INCR`+`EXPIRE`                                                                         |
