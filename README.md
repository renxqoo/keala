...
| ---------------------- | --------------------------------------------------------------------------- | ------------------- |
| `@honu/core`                 | Honu / Router / compose / Context / errors / cookies / bodyParser / helpers | the app surface     |
| `@honu/core/middleware`      | every middleware factory in one import                                      | the middleware tier |
| `@honu/core/middleware/cors` | one factory                                                                 | that file only      |
| `@honu/core/node`   | the Node listener (bun/node are exclusive)                                  | that file only      |
...
import {
  ...
  serveStatic,
  validator,
} from "@honu/core/middleware"; // the aggregate — or per file: honu/middleware/cors
...
| `app.listen(port?, host?, cb?)`                                                | Boots `Bun.serve`; returns the Bun `Server` (with `reload()`); `onServeError` optional override of the 500 handler. Under Node use `listen()` from `@honu/core/node` |
