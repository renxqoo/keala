 ...
 * Migrated to the current API (Honu from core/app, app.onError, Runtime
...
const drive = (app: InstanceType<typeof Honu>, url: string, init?: RequestInit) =>
...
describe("audit: x-forwarded-* trust chain", () => {
  ...
  it("maxIpsCount truncates the forwarded list from the right", async () => {
    const app = new Honu({ ...quiet, proxy: true, maxIpsCount: 1 });
    let captured: Context | undefined;
    app.use((c) => {
      captured = c;
      c.body = "ok";
