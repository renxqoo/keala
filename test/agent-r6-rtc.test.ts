 ...
 *         `new Eleu({ onStreamError })`. Without onStreamError the same
...
describe("R6-A cache(): r5 state-mode materialization vs the stream observer [RED]", () => {
  ...
  it("stream body + onStreamError + cache() must stream, not 500 (R6-1)", async () => {
    const seen: string[] = [];
    const app = new Eleu({ ...quiet, onStreamError: (e) => seen.push(e.message) });
    app.get("/s", cache(), (c) => {
  ...
  it("c.body = new Response(stream) hits the same lock (R6-1b)", async () => {
    const app = new Eleu({ ...quiet, onStreamError: () => undefined });
    app.get("/s", cache(), (c) => {
