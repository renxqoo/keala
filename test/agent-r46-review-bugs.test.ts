...
describe("R4.6 review hunt: signal bridge under real signals", () => {
  it(
    ...
    async () => {
      ...
      const childPath = fileURLToPath(new URL("./agent-r46-review-signal-child.ts", import.meta.url));
      const child = spawn(
        process.execPath,
      ...
      const portLine = await new Promise<string>((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error(`child never reported a port: ${out} ${err}`)), 8000);
...
describe("R4.6 review hunt: counter accounting at the admission/settle seam", () => {
  ...
  it("REVIEW-BUG-3: queue timeout racing slot transfer yields exactly one outcome and clean accounting", { timeout: 8000 }, async () => {
  ...
  it("REVIEW-BUG-4: drain-clear racing queue timeout rejects each waiter exactly once", { timeout: 8000 }, async () => {
  ...
  it("REVIEW-BUG-5: waiter pool churn with mid-queue aborts and timeouts never misattributes a response", { timeout: 10000 }, async () => {
...
describe("R4.6 review hunt: strategy × drain interleavings (U1)", () => {
  it("REVIEW-BUG-6: an untaken null landing after drain is refused; a slot taken before drain stays served", { timeout: 8000 }, async () => {
...
describe("R4.6 review hunt: deadline zombie × drain hold × pooling", () => {
  it("REVIEW-BUG-7: a pooling deadline zombie settling during drain neither double-releases nor retires into the pool", { timeout: 8000 }, async () => {
...
describe("R4.6 review hunt: Node adapter wire truth", () => {
  it("REVIEW-BUG-8: client abort mid-drain — wire clears early, close completes on the app counter", { timeout: 8000 }, async () => {
  ...
  it("REVIEW-BUG-9: a late keep-alive request during drain wedges close onto the drain timer — {timedOut:true} and a force-close although app and wire both reached zero", { timeout: 12000 }, async () => {
  ...
  it("REVIEW-BUG-10: a malformed request-target (InvalidRequestTarget) does not wedge stopGraceful (no wire leak)", { timeout: 8000 }, async () => {
...
describe("R4.6 review hunt: closeApp against hostile adapters", () => {
  it("REVIEW-BUG-11: escalation while stopGraceful is pending forces once; a rejecting stopGraceful cannot hang close; repeat close after completion is inert", { timeout: 8000 }, async () => {
  ...
  it("REVIEW-BUG-12: drain:0 as the FIRST close rejects a non-empty queue exactly once each", { timeout: 8000 }, async () => {
    const app = new Keala({
      env: "test",
...
describe("R4.6 review hunt: WS upgrade settlement (§2.2 r5)", () => {
  it("REVIEW-BUG-13: an upgrade settles at the 101 — no lingering concurrency slot, no drain hold, queue admission intact", { timeout: 8000 }, async () => {
...
describe("R4.6 review hunt: FLAG_DEADLINE_FIRED re-arm through a mapper takeover", () => {
  it("REVIEW-BUG-14: reading c.signal after a mapper-restyled 504 still yields an aborted signal", { timeout: 8000 }, async () => {
...
describe("R4.6 review hunt: Bun adapter escalation through the real close path", () => {
  it("REVIEW-BUG-15: escalating a pending stopGraceful via a second close({drain:0}) forces the Bun server (stop(undefined) then stop(true))", { timeout: 8000 }, async () => {
    const app = new Keala({ env: "test" });
