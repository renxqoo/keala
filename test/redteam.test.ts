...
//           baseContextProto (src/core/context/context.ts L98); Honu never
...
describe("redteam — RT-6 decorate leaks across apps", () => {
  it("CONFIRMED-BUG(now fixed) (RT-6a): decorate must not leak into other apps' contexts", async () => {
    const appA = new Honu(quiet);
    const appB = new Honu(quiet);
...
//
// Repro:    const app = new Honu(); app.get("/a", ...); app.mount("/self", app)
