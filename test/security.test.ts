...
const drive = (app: InstanceType<typeof Eleu>, request: Request) => app.handle(request);
...
describe("information disclosure", () => {
  it("never leaks stack traces or internals on 5xx", async () => {
    const app = new Eleu({ env: "production" });
...
describe("cookie integrity", () => {
  it("rejects forged signatures", async () => {
    const app = new Eleu({ keys: ["production-key"] });
    app.use(async (c) => {
  ...
  it("does not accept cookies signed with a retired key as new signatures", async () => {
    const app = new Eleu({ keys: ["new-key", "old-key"] });
    app.use(async (c) => {
      if (c.path === "/set") {
