# eleu — deliberate divergences (supersedes the parity-security (archived residue) ledger below)
...
| Node runtime             | —                                                              | official adapter `eleu/node` (streaming body bridge, pipeline backpressure, set-cookie fanout, 400/500/501 failure surfaces; ws stays Bun-only)                                |
...
| Upstream file                      | cases | status  | eleu verification                                                                                                                                                              |
| ---------------------------------- | ----: | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| request/accept.test.js             |     2 | N/A     | ctx.accept negotiator instance API; eleu exposes accepts()/accepts*() instead                                                                                                  |
...
- **Multiple controllers for multiple matching routes** (@koa/router runs every
  matching layer): eleu dispatches the single best match (static > param >
  wildcard). Multi-match chains would restructure the dispatch hot path.
- **`router.use()` timing (gh-182)**: eleu runs router middleware on every
