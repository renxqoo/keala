...
| cookie Path default      | `/` (the `cookies` package)                                    | `/` since R4.10 — 0.6 through 0.7.0 emitted no Path, silently scoping cookies to RFC 6265's default-path                                                                                                                       |
| consumed-Response reuse  | —                                                              | unified-loud (R4.10): a re-sent consumed Response is a FRAMED 500 on both runtimes — Node's degrade branch used to answer unframed and kill the keep-alive socket                                                              |
| cache capture surface    | —                                                              | framework snapshots only (sugar/state-mode text); streamed and hand-built bodies never captured; byte-budgeted via maxBytes/maxEntryBytes (R4.10)                                                                              |
