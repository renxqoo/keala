...
| D3  | 双运行时保留                                 | 核心代码不引用 `Bun` 全局；Node + Bun 双跑测试；bench 三方对比（eleu / hono / raw）。                                                                                                  |
...
基准纪律：ABAB 交错批（eleu/raw/hono 同进程同批次循环，每框架 ≥5 批取中位+IQR）；预热至连续 3 批变异 <2%；drain 必须消费 body（堵假快）；整套 ≥3 遍取中位；判定带 ±5% 噪声带；Bun 版本/机器/日期写入 BENCH.md。

| 门禁                        | 判定                                                                      | 阶段  |
| --------------------------- | ------------------------------------------------------------------------- | ----- |
| G1 进程内 text              | eleu ≥ 0.90 × 同 run raw                                               | P1    |
| G2 进程内 param             | eleu ≥ 1.05 × 同 run hono                                              | P1    |
| G3 分配预算                 | param 路径每请求 JS 分配 ≤3（现状 ~10）                                   | P1    |
| G4 HTTP 吞吐                | eleu ≥ 0.98 × hono（100 与 1000 路由、hit+miss）                       | P1/P4 |
| G5 p99 延迟                 | eleu ≤ 1.10 × hono                                                     | P4    |
| G6 路由规模                 | 1000 路由（含**共享首段**形态与 404 miss）eleu ≥ 4 × hono              | P1    |
...
核心（app/compose/context/router/respond/utils）不引用 `Bun` 全局——`Bun.serve`/`requestIP`/`routes` 全部经 `adapters/bun.ts` 注入；Node 下 `app.handle` 完整可用（vitest 双跑）；Bun-only 组件（websocket/nativeSink/responseCache 的 Date 处理等）在 Node 下 `install()` 抛明确错误或跳过（`skipIf(!isBun)` 测试模式）。bench 对比固定三方：eleu / hono（同版本钉死）/ raw，双运行时数据并列展示。
