...
1. **R4.1 committed header Fast 通道**：当前唯一有纳秒证据且契约可封闭的单元；
2. **R4.2 路由规模自适应研究**：先基准与算法裁决，确认收益后才实现；
...
- **默认裁决（待否决）**：第一实现单元只做 committed header Fast 通道；不先重写
  router/compose，不新增公开 API。
- **默认裁决（待否决）**：现有 `onError` 永久保留观察语义；错误响应策略另命名。
- **默认裁决（待否决）**：性能竞争以同语义为准，不用关闭 413 安全预算制造胜利。
