# bun-koa v2 — 设计文档（定稿 v2.1）

状态：已按 4 路子代理审计（性能/安全/功能/迁移）修订，用户已批准三项关键决策。
前置阅读：`docs/v2-AUDIT.md`（审计裁决记录）、`docs/v2-MIGRATION.md`（逐条迁移矩阵）。
...
Bun 1.4+ 专属的高性能 Web 框架：**hono 的性能与功能面 + koa 的洋葱中间件人体工学**，零依赖、全函数式（无 class）、每文件 ≤500 行。不兼容 v1 API（同仓库重写，v1 有 git 快照 `dd44353`）。
...
Bun.serve fetch(request, server)
  └─ app.handle(request, runtime)          ← runtime: {server?, remote?, env?} 注入通道
       ...
       │     b) 链 → 嵌套预编译层（v1 compose 语义，double-next 守卫保留）
       ├─ 4. 响应终结（双模，见 §4）
...
与 v1 的结构差异：路由从"洋葱的一层"提升为顶层先行（省 76~170ns/请求）；`app.use` 全局中间件在**注册期**编译进每条路由链首（含未命中路径的兜底链）；晚于路由注册的 `use()` 触发全部链 O(routes) 重编（文档明示成本）。

## 3. Context：单对象扁平化

每请求 **1 个对象**（v1 为 4 个），字段一次成型（固定隐藏类）：
...
要点：`c.header(name, value)` 直写 `headers` record（惰性创建）；委托链层数 = 0（v1 有 40 处两跳委托）；`c.state`/`c.throw`/`c.assert`/`c.cookies`（含 signed + keys 轮换）**保留**（koa 人体工学，审计确认必须保留项）。
...
匹配（顶层，优先级）:
  ...
  3. rootTrie                                       // 复用 v1 trie（规模化 + wildcard + optional）
  4. 首段即参数（/:org/:repo）→ 专用 paramBuckets

正确性规格（从 v1 trie 继承，配 48 例 router-edge 测试）：
...
1. 错误路径清头（除 set-cookie 全删）+ 5xx 隐藏 message + `error.headers` 逐个过校验、失败静默丢弃（v1 app.ts:286-308）
...
基准纪律：ABAB 交错批（v2/raw/hono 同进程同批次循环，每框架 ≥5 批取中位+IQR）；预热至连续 3 批变异 <2%；drain 必须消费 body（堵假快）；整套 ≥3 遍取中位；判定带 ±5% 噪声带；Bun 版本/机器/日期写入 BENCH.md。

| 门禁                        | 判定                                                                      | 阶段  |
| --------------------------- | ------------------------------------------------------------------------- | ----- |
| G1 进程内 text              | v2 ≥ 0.90 × 同 run raw                                                    | P1    |
| G2 进程内 param             | v2 ≥ 1.05 × 同 run hono                                                   | P1    |
| G3 分配预算                 | param 路径每请求 JS 分配 ≤3（现状 ~10）                                   | P1    |
| G4 HTTP 吞吐                | v2 ≥ 0.98 × hono（100 与 1000 路由、hit+miss）                            | P1/P4 |
| G5 p99 延迟                 | v2 ≤ 1.10 × hono                                                          | P4    |
| G6 路由规模                 | 1000 路由（含**共享首段**形态与 404 miss）v2 ≥ 4 × hono                   | P1    |
| G7 并发                     | Promise.all 批量 in-flight 下 G1/G2 不回退                                | P1    |
| G8 流/SSE                   | 流响应无 observeStream 税（opt-in 关闭时 ≤ 1.1× raw stream）              | P2    |
| G9 JSON                     | `c.json()` 走 Response.json 杠杆（≥1.15× v1）                             | P1    |
| G10 缓存命中（opt-in 组件） | ≥ 0.80 × 同 run raw                                                       | P3    |
| G11 内存                    | 稳态 RSS/heap ≤ v1；soak 三层漂移 <0.5%                                   | P4    |
| G12 结构围栏                | 零 Promise 同步链、隐藏类键序稳定、字节预算（agent-perf-evidence 重标定） | P1    |

## 9. 双运行时（D3）

核心（app/compose/context/router/respond/utils）不引用 `Bun` 全局——`Bun.serve`/`requestIP`/`routes` 全部经 `adapters/bun.ts` 注入；Node 下 `app.handle` 完整可用（vitest 双跑）；Bun-only 组件（websocket/nativeSink/responseCache 的 Date 处理等）在 Node 下 `install()` 抛明确错误或跳过（`skipIf(!isBun)` 测试模式）。bench 对比固定三方：v2 / hono（同版本钉死）/ raw，双运行时数据并列展示。
...
| P1   | 核心重写：管线/Context/路由混合/终结器 + 11 条契约绑定测试迁移 + bench 基建 v2 化                             | G1/G2/G3/G6/G7/G9/G12 + 迁移套件 ≥550 例绿（构成见 MIGRATION §2） | 3-5 天 |
...
红队纪律：写入白名单仅 `test/redteam-v2/`；账本四段式（repro/expect/actual/root）；入口=功能全绿；出口=连续一轮零新增高危；170+ 例安全语料全程保持绿。
