...
| `cors` / `csrf`                       | 携带凭证时要求 origin 白名单；反射 origin 始终带 `Vary: Origin`（恒定回答 `*` 的不带 —— 它永不变化）；拒绝 `Origin: null`；只处理真正的预检（带 `Access-Control-Request-Method` 的请求）；`origin` 也接受逐请求判定的函数；预检反射 `Access-Control-Request-Headers` 需显式开启（`reflectHeaders`）；`csrf({ allow })` 在 origin 校验前豁免已认证的非浏览器调用方 |
| `etag` / `compress`                   | 弱 tag + 304（If-None-Match 优先级遵循 RFC 9110）；异步 gzip，gzip 实现可注入用于测试                                                                                                                                                                                                                                                                             |
| `serveStatic`                         | 解码 → 归一化 → 包含性检查；NUL 字节 400；出现任一符号链接组件即拒绝（目录也算）；index 路径重新做包含性检查；`precompressed` 服务 `.br`/`.gz` 预压缩版本（优先 brotli，命中带 `Content-Encoding` + `Vary: Accept-Encoding`）；`onFound`/`onNotFound` 钩子                                                                                                        |
| `rateLimit`                           | 固定窗口 per-key（默认 `c.ip`）；429 + `Retry-After` + `RateLimit-*` 头；共享 store 实现原子 `{ hit, get }` 接口——**0.8 破坏性：不再直接接受裸 `Map`**（get/set 读-改-写会让多 worker 同时越过限值）；用 `memoryRateLimitStore(maxKeys)` 包装，Redis 适配器对应 `INCR`+`EXPIRE`                                                                                   |
