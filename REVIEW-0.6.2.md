...
- **合并 0.6.2（852ce30，2026-09-05 18:3x）**：并入 ab0a3c4（R413b wildcard 快层回退 + sync-settle deadline skip）、b67e206/84034d0（diag 脚本格式 + lint 修复）。唯一冲突 profile-loop.ts 为双方格式化撞车，取 0.6.2 侧；与本轮改动文件面零重叠。合并后重跑全量门禁：171 文件 / 2647 通过 + 11 跳过、0 失败，覆盖率 95.41% / 91.06% / 96.36% / 97.07%，smoke + example-check 通过。
...
- **bench-zz 装置已于 0486dc9 移除**（验证完毕；无 package.json/CI/测试活引用）。本节数字为最终口径；PERF-1 pooling 重构如需复测，`git checkout 369528b -- bench-zz` 原样恢复。
